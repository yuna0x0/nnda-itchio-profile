...
function cmd(command: string, args: string[], errMsg: string): void {
  ...
  if (result.status !== 0) throw new Error(`Error ${result.status}: ` + errMsg);
}
...
(async (): Promise<void> => {
  ...
  function getModified(status: StatusResult, files: string[]): string[] {
  ...
  try {
    ...
    ["dist/styles.css", "dist/i.css"].forEach((file: string): void => {
      cmd("npx", ["csstree-validator", file], "CSS output validation failed");
    });
    ...
    cmd("git", ["commit", "-m", version], "git commit failed");
