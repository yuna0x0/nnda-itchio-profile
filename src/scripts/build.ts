...
import { getCSSRulesArr } from "./optimize/util";

import cdnify from "./cdnify";

import { ghSlug, readTxtFile, writeTxtFile } from "./util";

import html from "./optimize/html";

(async (): Promise<void> => {
  const rawCSS: string = await nano(readTxtFile("./src/styles.css"), true),
  ...
  writeTxtFile("dist/content.html", cdnify(await html(), repoUser, version));
  ...
  whyDoesItchIoProfilePageHave5120CharacterCountLimitButOtherPagesDontIDontUnderstand(
    ...
    writeTxtFile(
      "dist/" + (imported ? "i.css" : "styles.css"),
      (!imported ? CDNImportTemplate : "") + cdnify(css, repoUser, version),
    );

    if (!imported) {
      console.log(" total:", cssAll.length);

      writeTxtFile("dist/test.css", cssAll);
...
+ ----------------------------------------------- +
|                                                 |
|  Run 'pnpm run publish' to push the updates.     |
